...
class TweetListHeader {
  ...
  Future<void> refreshTweetsFromUser(TweetUserEncode tweetUser, {bool isAuto = false}) async {
    ...
    if (allTweetsFromUser != null) {
      ...
      if (lastId > tweetUser.sinceId) {
        ...
        unawaited((db.update(db.tweetUser)..where((tbl) => tbl.tweetUserId.equals(tweetUser.tweetUserId))).write(TweetUserCompanion(sinceId: Value(lastId))).then((value) => debugPrint('Updated db: $value')));
  ...
  Future<void> updateTweetsFromUser(TweetFullDecode tweetFullDecode) async {
    ...
    for (var i = 0; i < tweetFullDecode.tweets.length; i++) {
      ...
      if (!tweets.any((element) => tweetFullDecode.tweets[i].id == element.id)) {
        ...
        unawaited(db.insertTweet(tweetFullDecode.tweets[i]).then((value) => _insertNewTweetToList(tweetFullDecode.tweets[i])));
  ...
  Future<void> removeTweet(int twitterId) async {
    unawaited(db.updateTweetStatus(twitterId));
    ...
    if (index >= 0) {
      ...
      _listTweetKey.currentState!.removeItem(index, (context, animation) => SizeTransition(sizeFactor: animation, child: TwitterWidget.tweetContainer(context, cachedItem)), duration: const Duration(milliseconds: 500));
